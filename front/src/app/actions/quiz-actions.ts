...
export async function createQuizAction(data: { title: string; timePerQuestion: number; passingScore: number; modeRandom: boolean }) {
    const session = await auth.api.getSession({ headers: await headers() });
    if (!session) return { success: false, error: "Unauthorized" };
...
export async function updateQuizAction(id: number, data: { title: string; timePerQuestion: number; passingScore: number; modeRandom: boolean }) {
    const session = await auth.api.getSession({ headers: await headers() });
    if (!session) return { success: false, error: "Unauthorized" };
...
export async function updateQuestionAction(id: number, data: {
    text: string;
    quizId: number;
    explanation: string;
    correctAnswer: string;
