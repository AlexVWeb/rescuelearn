...
import { Slot, Inscription, EmargementStatus } from "../../../types";
...
export function EmargementTab({ sessionId, sessionTitle, sessionLocation, sessionType, slots, inscriptions }: EmargementTabProps) {
  ...
  const getEmargement = (inscription: Inscription) => {
    return inscription.emargements?.find((e: any) => e.slotId === selectedSlot) || { status: "en_attente" };
  };

  if (slots.length === 0) {
    return <div className="text-center p-4">Veuillez d'abord configurer des créneaux dans l'onglet Programme.</div>;
  }

  if (inscriptions.length === 0) {
    return <div className="text-center p-4">Aucun stagiaire inscrit à cette session.</div>;
  }
  ...
  const sampleEmargementWithPin = inscriptions[0]?.emargements?.find((e: any) => e.slotId === selectedSlot && e.validationCode);
  const currentPin = sampleEmargementWithPin?.validationCode || "Aucun code PIN actif";
  ...
  return (
    <div className="space-y-6">
          ...
          <SelectContent>
            {slots.map(slot => (
              <SelectItem key={slot.id} value={slot.id}>
                {slot.label} ({new Date(slot.date).toLocaleDateString()} {slot.startTime}-{slot.endTime})
              </SelectItem>
            ))}
          </SelectContent>
        </Select>

        <div className="flex items-center gap-4 bg-muted/50 px-4 py-2 rounded-lg border w-full sm:w-auto">
          <div className="flex flex-col">
            <span className="text-xs text-muted-foreground uppercase font-bold tracking-wider">PIN d'émargement</span>
            <span className="text-2xl font-mono tracking-widest">{currentPin}</span>
          </div>
          <Button variant="outline" size="icon" onClick={handleGeneratePin} disabled={loading} title="Générer un nouveau PIN">
      ...
      <div className="flex justify-end mb-2">
        <Button variant="outline" onClick={() => generateEmargementPDF({ title: sessionTitle, location: sessionLocation, type: sessionType }, slots, inscriptions)}>
                ...
                <TableRow key={inscription.id}>
                  ...
                  <TableCell className="text-right">
                    <Select 
                      ...
                      onValueChange={(val) => handleStatusChange(inscription.id, val)}
                      disabled={loading}
                    >
