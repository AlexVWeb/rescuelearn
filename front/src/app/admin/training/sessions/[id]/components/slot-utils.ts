...
export function getNextSlotSuggestion(
  ...
  if (slots.length === 0) {
    if (sessionStartDate && sessionEndDate && dayjs(defaultDate).isAfter(dayjs(sessionEndDate), 'day')) {
      return null;
    }
    return {
      label: "Jour 1 - Matin",
  ...
  const hasMorning = slotsOnLastDate.some(s => parseInt(s.startTime.split(':')[0]) < 13);
  const hasAfternoon = slotsOnLastDate.some(s => parseInt(s.startTime.split(':')[0]) >= 13);
  ...
  // Vérifier si le jour suivant est toujours dans la session
  if (sessionEndDate && nextDate.isAfter(dayjs(sessionEndDate), 'day')) {
    return null;
  }
