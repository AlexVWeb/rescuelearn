...
describe("slot-utils", () => {
  ...
  describe("isDateWithinSession", () => {
    it("should return true if date is within session range", () => {
      expect(isDateWithinSession("2024-04-01", sessionStart, sessionEnd)).toBe(true);
      expect(isDateWithinSession("2024-04-02", sessionStart, sessionEnd)).toBe(true);
      expect(isDateWithinSession("2024-04-03", sessionStart, sessionEnd)).toBe(true);
    });

    it("should return false if date is outside session range", () => {
      expect(isDateWithinSession("2024-03-31", sessionStart, sessionEnd)).toBe(false);
      expect(isDateWithinSession("2024-04-04", sessionStart, sessionEnd)).toBe(false);
    });
  ...
  describe("getNextSlotSuggestion", () => {
    ...
    it("should return null if the next suggested day is after sessionEnd", () => {
      ...
      const suggestion = getNextSlotSuggestion(slots, sessionStart, sessionEndShort);
    ...
    it("should handle potential timezone shifts during suggestion", () => {
      ...
      const suggestion = getNextSlotSuggestion(slots, dayjs("2026-03-16").toDate(), dayjs("2026-03-20").toDate());
