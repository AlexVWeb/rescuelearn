    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center rounded-md border border-yellow-200 bg-yellow-100 px-2.5 py-1 text-xs font-medium text-yellow-800">
            Tableau mnémotechnique
          </span>
          <p className="text-sm text-gray-600">
            Utilisez ce tableau pour mémoriser rapidement les libellés. Le
            tableau réel se trouve plus bas et peut être affiché/masqué.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button
            onClick={() => setShowRealTable((v) => !v)}
            variant={showRealTable ? "secondary" : "default"}
            aria-expanded={showRealTable}
            aria-controls="real-glasgow-table"
          >
      ...
      <div
        id="real-glasgow-table"
        className={showRealTable ? "block" : "hidden"}
      >
        <RealGlasgowTable />
      </div>
    </div>
