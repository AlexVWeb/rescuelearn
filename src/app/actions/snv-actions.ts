  ...
  try {
    const [scenarios, total] = await Promise.all([
      prisma.sNVScenario.findMany({
        // Capitalization might vary, checking Prisma schema: model SNVScenario
        where,
        skip,
        take: limit,
        orderBy: { id: "desc" },
...
export async function createVictimAction(data: {
  description: string;
  correctAnswer: number;
  explanation: string;
  scenarioId: number;
...
export async function updateVictimAction(
  id: number,
  data: {
    description: string;
    correctAnswer: number;
